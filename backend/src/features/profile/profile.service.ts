...
export const getProfile = async (userId: mongoose.Types.ObjectId) => {
  const user = await User.findOne({ _id: userId, userType: { $in: ['Manager', 'Landlord'] }, status: 'verified' }).lean() as ManagerType;
  if (user.userType == 'Manager') {
    const facilities = await HousingFacility.find({ managers: { $elemMatch: { userId } } }) as HousingFacilityType[];
    ...
    const employers = landlords.map(x => ({ id: x._id, firstName: x.firstName, middleName: x.middleName, lastName: x.lastName }));
    ...
    const facilities = await HousingFacility.find({ landlordId: userId }) as HousingFacilityType[];
    const leanFacilities = facilities.map(x => ({ id: x._id, name: x.name }));
    ...
    const employees = managers.map(x => ({ id: x._id, firstName: x.firstName, middleName: x.middleName, lastName: x.lastName }));
