...
import { adminAgent, studentAgent, guestAgent } from './setup.js';
  ...
  describe('POST /api/transfers', () => {
    describe('Authentication', () => {
      ...
      it('should return 403 for landlord', async () => {
        const response = await guestAgent.post('/api/auth/test/login').send({
          email: 'landlord-transfer@test.com',
        });
        const landlordAgent = response;
        const transferResponse = await (landlordAgent as any).post('/api/transfers').send({
          unitID: unitId,
        });
