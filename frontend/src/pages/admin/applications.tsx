        ...
        <div className="flex flex-1 relative z-10 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto bg-transparent px-10 py-8">
            ...
            <div className="mt-6 rounded-xl bg-white dark:bg-[#141515] p-6 shadow-sm border border-transparent dark:border-[#303331]">
              ...
              {error && (
                <div className="mb-4 rounded-xl border border-red-100 dark:border-red-900/50 bg-red-50 dark:bg-red-900/20 px-4 py-3 font-['Poppins'] text-sm font-semibold text-red-700 dark:text-red-400">{error}</div>
              )}
              {actionMessage && (
                <div className="mb-4 rounded-xl border border-emerald-100 dark:border-emerald-900/50 bg-emerald-50 dark:bg-emerald-900/20 px-4 py-3 font-['Poppins'] text-sm font-semibold text-emerald-700 dark:text-emerald-400">{actionMessage}</div>
              )}
              ...
              <div className="overflow-hidden rounded-2xl shadow-[0px_0px_20px_0px_rgba(0,0,0,0.35)] dark:border dark:border-[#303331]">
                <table className="w-full">
                  ...
                  <tbody>
                    {isLoading ? (
                      <tr><td colSpan={tableHeaders.length} className="px-6 py-8 text-center dark:text-[#a4acba]">Loading applications...</td></tr>
                    ) : paginatedApplicants.length === 0 ? (
                      <tr><td colSpan={tableHeaders.length} className="px-6 py-8 text-center dark:text-[#a4acba]">No submitted verification applications.</td></tr>
                    ) : (
                      paginatedApplicants.map((row) => (
                        <tr key={row._id} className="border-b border-[#f0f0f0] dark:border-[#303331] bg-white dark:bg-[#141515]">
                          <td className="px-6 py-3 font-['Poppins'] text-[16px] font-medium text-black dark:text-[#d7e0ef]">{getDisplayName(row)}</td>
                          <td className="px-6 py-3 font-['Poppins'] text-[16px] font-medium text-black dark:text-[#d7e0ef]">{row.emails?.[0] ?? 'No email'}</td>
                          <td className="px-6 py-3 font-['Poppins'] text-[16px] font-medium text-black dark:text-[#d7e0ef]">{formatRole(row.userType)}</td>
                          <td className="px-6 py-3 font-['Poppins'] text-[16px] font-medium text-black dark:text-[#d7e0ef]">{row.documents?.filter((d) => d.files.length > 0).length ?? 0}</td>
                          <td className="px-6 py-3 font-['Poppins'] text-[16px] font-medium text-black dark:text-[#d7e0ef]">{getApplicationStatusLabel(row)}</td>
