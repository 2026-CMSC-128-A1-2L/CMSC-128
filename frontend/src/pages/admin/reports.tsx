          ...
          <div className="flex-1 overflow-y-auto bg-transparent px-10 py-8">
            ...
            <div className="mt-6 rounded-xl bg-white dark:bg-[#141515] p-6 shadow-sm border border-transparent dark:border-[#303331]">
              <div className="mb-4 flex items-center justify-between">
                <h2 className="font-['Poppins'] text-[36px] font-bold text-[#001d18] dark:text-[#d7e0ef] drop-shadow-[0px_4px_4px_rgba(0,0,0,0.1)]">Reports</h2>
              ...
              <div className="overflow-hidden rounded-2xl shadow-[0px_0px_20px_0px_rgba(0,0,0,0.35)] dark:border dark:border-[#303331]">
                <table className="w-full">
                  ...
                  <tbody>
                    {isLoading ? (
                      <tr><td colSpan={tableHeaders.length} className="px-6 py-8 text-center font-['Poppins'] text-[#7c8db5] dark:text-[#a4acba]">Loading reports...</td></tr>
                    ) : paginatedReports.length === 0 ? (
                      <tr><td colSpan={tableHeaders.length} className="px-6 py-8 text-center font-['Poppins'] text-[#7c8db5] dark:text-[#a4acba]">No reports found.</td></tr>
                    ) : (
                      paginatedReports.map((report) => (
                        <tr key={report._id} className="border-b border-[#f0f0f0] dark:border-[#303331] bg-white dark:bg-[#141515] transition-colors duration-200 hover:bg-[#f8fffe] dark:hover:bg-[#17201d]">
                          <td className="px-6 py-3 font-['Poppins'] text-[16px] font-medium text-black dark:text-[#d7e0ef]">{getReporterName(report.userId)}</td>
                          <td className="max-w-xs truncate px-6 py-3 font-['Poppins'] text-[16px] font-medium text-black dark:text-[#d7e0ef]">{report.description.replace(/\[.*?\]\s*/, '').slice(0, 60)}...</td>
                          <td className="px-6 py-3 font-['Poppins'] text-[16px] font-medium text-black dark:text-[#d7e0ef]">{getReportType(report)}</td>
