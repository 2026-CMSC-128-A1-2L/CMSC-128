...
const FinancePopup: FunctionComponent<PopupProps> = ({
  children,
  overlayColor,
  placement = 'Centered',
  onOutsideClick,
  ...
  const onOverlayClick = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      if (onOutsideClick && (e.target as HTMLElement).classList.contains('FinancePopupOverlay')) {
        onOutsideClick();
      }
      e.stopPropagation();
    },
  ...
  return (
    <Portal>
      <div
        className="flex flex-col fixed inset-0 FinancePopupOverlay"
        style={popupStyle}
