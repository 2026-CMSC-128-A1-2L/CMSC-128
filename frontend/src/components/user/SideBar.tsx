...
export type SideBarItemKey =
  | 'home' | 'messages' | 'bookmarks' | 'calendar' | 'finance' | 'settings';
      ...
      <div className="w-full px-4">
        {collapsed ? (
          <div className="flex justify-center">
            <button type="button" aria-label="Search" className="flex h-10 w-10 items-center justify-center rounded-full bg-[#f5f5f5] hover:bg-gray-200 transition-colors">
      ...
      <nav className="flex w-full flex-col gap-3">
        {navItems.map((item) => {
          const state: SideBarButtonState =
            ...
            item.key === internalHover ? 'hovered' : 'default';
      ...
      <div className="flex flex-1 w-full flex-col justify-end gap-3">
        ...
        >
          ...
          <span className="flex items-center gap-4 rounded-xl px-1">
            ...
            {!collapsed && <span className="font-semibold text-[14px] text-[#2d3748]">Dark Mode</span>}
          </span>
        </button>
        ...
        >
          ...
          {!collapsed && (
            <div className="flex flex-col items-start gap-1 overflow-hidden">
              <b className="text-[14px] text-[#2d3748]">{user?.name ?? 'Sign In'}</b>
              {!user?.signedIn && (
                <span className="text-[10px] text-[#9ca3af] font-bold">to continue</span>
              )}
            </div>
          )}
        </button>
