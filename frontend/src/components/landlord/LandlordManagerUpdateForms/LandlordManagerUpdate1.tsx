...
const PERMISSIONS = [
  ...
  },
  {
    ...
    items: [
      { icon: 'solar:calendar-bold', title: 'Accept Ocular Visits', desc: 'Accept ocular visit requests' },
...
const UpdateManager1: FunctionComponent<Props> = ({ onCancel, onSave }) => {
  ...
  const toggleSection = (keys: string[]) => setChecked((p) => { const n = new Set(p); const allOn = keys.every((k) => n.has(k)); keys.forEach((k) => allOn ? n.delete(k) : n.add(k)); return n; });
  const toggleAll = () => setChecked(checked.size === ALL_KEYS.length ? new Set() : new Set(ALL_KEYS));
      ...
      <div className="self-stretch flex flex-col items-start px-[38.4px] gap-[17.6px] overflow-y-auto flex-1 pt-[22px] pb-[10px]">
        ...
        <div className="self-stretch flex flex-col items-start gap-[6.4px]">
          ...
          <div className="self-stretch h-[38.4px] rounded-[9.6px] border border-[#f5f5f5] flex items-center px-[12.8px]">
            <span className="font-['Inter',sans-serif] text-[14px] font-medium text-black">ncunanan@gmail.com</span>
          </div>
        </div>
                    ...
                    >
                      <div className="flex flex-1 items-center gap-[12.8px]">
                        ...
                        <div className="flex flex-col gap-[3.2px]">
                          ...
                          <span className="font-['Inter',sans-serif] text-[12px] font-medium text-[#666]">{item.desc}</span>
                        </div>
                      </div>
