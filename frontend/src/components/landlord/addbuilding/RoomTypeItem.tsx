  ...
  return (
    ...
    >
      <input
        ...
        onChange={(e) =>
          updateTagValue(roomTypeId, tag.tagId, Number(e.target.value))
        }
        placeholder="–"
...
const TagsSection: FunctionComponent<{ roomType: RoomTypeData }> = ({
  roomType,
}) => {
  const { addTag, removeTag, updateTagValue } = useBuildingStore();
  const [activeCategory, setActiveCategory] = useState(
    TAG_CATALOGUE[0].category,
  );
  ...
  const currentTags =
    TAG_CATALOGUE.find((c) => c.category === activeCategory)?.tags ?? [];
  ...
  return (
    <div className="self-stretch flex items-center gap-6">
      ...
      <div className="flex-1 flex flex-col items-center justify-center gap-2.5">
        <div className="relative leading-6 font-medium text-black text-sm dark:text-[#a4acba]">
          Room Number
        </div>
...
const RoomsSection: FunctionComponent<{ roomType: RoomTypeData }> = ({
  roomType,
}) => {
      ...
      <div className="flex items-center px-4 py-2">
        {!isExpanded && (
          <div className="flex-1 text-left">
            <b className="text-sm text-black dark:text-[#d7e0ef]">
              {headerLabel}
            </b>
          </div>
        )}
        ...
        <div className="flex flex-col px-4 pb-8 gap-6 text-sm text-gray-500 font-inter dark:text-[#a4acba]">
          ...
          <div className="self-stretch flex flex-col items-start gap-2.5">
            <div className="flex items-center gap-4">
              ...
              <span className="font-medium text-dimgray dark:text-[#6b7280]">
                Not required
              </span>
            </div>
            <textarea
