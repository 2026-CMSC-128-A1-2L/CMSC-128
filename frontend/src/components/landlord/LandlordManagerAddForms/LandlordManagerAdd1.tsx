...
const PERMISSIONS = [
  {
    ...
    items: [
      { icon: 'solar:trash-bin-trash-bold', title: 'Delete Buildings', desc: 'Allow manager to remove building records' },
      { icon: 'solar:trash-bin-trash-bold', title: 'Delete Listings', desc: 'Allow manager to remove listing records' },
      { icon: 'solar:home-bold', title: 'Manage Buildings', desc: 'Allow manager to access and edit building info' },
    ],
  },
  {
    section: 'Billings and Financials',
    items: [
      { icon: 'solar:bill-list-bold', title: 'Manage Billings', desc: 'Manage rents and utility payments' },
    ],
  },
  {
    section: 'Tenant Management',
    items: [
      { icon: 'solar:calendar-bold', title: 'Accept Ocular Visits', desc: 'Accept ocular visit requests from potential tenants' },
      { icon: 'solar:flag-bold', title: 'Report Users', desc: 'Report users for misconduct upon review' },
    ],
  },
];
...
const AddManager1: FunctionComponent<Props> = ({ onCancel, onSend }) => {
  ...
  const Checkbox = ({ active, onToggle }: { active: boolean; onToggle: () => void }) => (
    <button
      onClick={onToggle}
      className={`flex h-[20px] w-[20px] shrink-0 items-center justify-center rounded-[3px] border transition-colors ${
        active ? 'border-[#096c5b] bg-[#096c5b]' : 'border-[#d1d5db] bg-[#f5f5f5] shadow-[0px_0px_2px_rgba(0,0,0,0.25)]'
      }`}
      aria-checked={active}
      role="checkbox"
    >
      ...
      <div className="self-stretch flex flex-col items-start px-[38px] pb-[16px] gap-[18px]">
        ...
        <div className="self-stretch flex flex-col items-start gap-[6px]">
          ...
          <div className="self-stretch h-[38px] rounded-[10px] border border-[#f5f5f5] flex items-center px-[13px]">
            <span className="font-['Inter',sans-serif] text-[13px] font-medium text-black">ncunanan@gmail.com</span>
          </div>
        </div>
        ...
        <div className="self-stretch flex items-center justify-end gap-[9px] pr-[18px]">
          ...
          <Checkbox active={checked.size === ALL_KEYS.length} onToggle={toggleAll} />
        </div>
            ...
            <div key={group.section} className="self-stretch flex flex-col items-start gap-[16px]">
              <div className="self-stretch flex items-center justify-between pr-[18px]">
                ...
                <div className="flex items-center gap-[9px]">
                  <span className="font-['Inter',sans-serif] text-[11px] font-medium text-[#8a9099]">Select All</span>
                  <Checkbox active={allOn} onToggle={() => toggleSection(sectionKeys)} />
                </div>
              </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex flex-1 items-center gap-[13px]">
                        <div className={`flex h-[24px] w-[24px] shrink-0 items-center justify-center rounded-full transition-colors ${active ? 'bg-[#096c5b]' : 'bg-[#f5f5f5]'}`}>
                          <Icon icon={item.icon} className={`h-[12px] w-[12px] ${active ? 'text-white' : 'text-[#096c5b]'}`} aria-hidden="true" />
                        </div>
                      ...
                      <Checkbox active={active} onToggle={() => toggle(item.title)} />
                    </button>
                  );
                })}
              </div>
