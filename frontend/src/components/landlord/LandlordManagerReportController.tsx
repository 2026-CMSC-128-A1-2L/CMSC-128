...
const ReportManagerModal: FunctionComponent<Props> = ({ isOpen, onClose }) => {
  ...
  return (
    ...
    >
      {step === 1 && (
        <ReportManager1 onNext={() => setStep(2)} onCancel={handleClose} />
      )}

      {step === 2 && (
        <ReportManager2 onNext={() => setStep(3)} onCancel={handleClose} />
      )}

      {step === 3 && (
        <ReportManager3 onNext={() => setStep(4)} onCancel={handleClose} />
      )}

      {step === 4 && (
        <ReportManager4 onNext={() => setStep(5)} onCancel={handleClose} />
      )}

      {step === 5 && (
        <ReportManager5 onSubmit={() => setStep(6)} onCancel={handleClose} />
      )}
