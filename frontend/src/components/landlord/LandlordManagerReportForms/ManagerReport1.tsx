...
const ISSUES: ViolationOption[] = [
  { id: 'records', label: 'Mismanagement of Tenant Records', description: 'Lost, incomplete, or falsified data.' },
  { id: 'policies', label: 'Failure to Enforce Dorm Policies', description: 'Ignoring curfews, guest rules, etc.' },
  { id: 'decisions', label: 'Unauthorized Decision-Making', description: 'Acting without landlord approval.' },
  { id: 'negligence', label: 'Negligence in Duties', description: 'Not responding to tenant concerns or issues.' },
  { id: 'conflict', label: 'Conflict of Interest', description: 'Favoring certain tenants unfairly.' },
];

const ReportManager1: FunctionComponent<StepProps> = ({ onNext, onCancel }) => {
  ...
  const handleSelectAll = () => {
    if (selectedIssues.length === ISSUES.length) {
      ...
      setSelectedIssues(ISSUES.map(v => v.id));
    }
  };
          ...
          <div className="self-stretch flex flex-col items-start pt-6 px-8 pb-4 gap-8 text-[12px] text-dimgray font-inter">
            ...
            <div className="self-stretch flex flex-col items-start gap-2">
              <div className="self-stretch flex items-end py-0 pl-0 pr-4">
                <b className="self-stretch flex-1 relative flex items-center text-[12px]">Administrative & Management Issues</b>
                <button 
                ...
                >
                  ...
                  <div className="h-4 w-4 relative">
                    <div className={`absolute h-full w-full rounded-sm ${
                      selectedIssues.length === ISSUES.length ? 'bg-teal' : 'bg-whitesmoke-100 border border-gray-200'
                    }`} />
                    {selectedIssues.length === ISSUES.length && <CheckIcon />}
                  </div>
                </button>
              </div>
