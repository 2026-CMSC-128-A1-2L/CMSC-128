...
const MAINTENANCE_ISSUES: ViolationOption[] = [
  { id: 'cleanliness', label: 'Failure to Maintain Cleanliness', description: 'Common areas or facilities left in poor condition.' },
  { id: 'repairs', label: 'Ignoring Repair Requests', description: 'Long delays or refusal to fix reported issues.' },
  { id: 'staff', label: 'Improper Handling of Maintenance Staff', description: 'Poor coordination or treatment of repair crews.' },
  { id: 'hazards', label: 'Safety Hazards Not Addressed', description: 'Fire safety, structural, or electrical risks ignored.' },
];

const ReportManager3: FunctionComponent<StepProps> = ({ onNext, onCancel }) => {
  const [selectedIssues, setSelectedIssues] = useState<string[]>([]);

  const isNothingSelected = selectedIssues.length === 0;
  ...
  const handleSelectAll = () => {
    if (selectedIssues.length === MAINTENANCE_ISSUES.length) {
      setSelectedIssues([]);
    } else {
      setSelectedIssues(MAINTENANCE_ISSUES.map(v => v.id));
    }
  };
        ...
        <div className="self-stretch flex flex-col items-center">
          ...
          <div className="self-stretch rounded-tl-[24px] [background:linear-gradient(183.48deg,_#096c5b,_#16917c)] flex flex-col items-start justify-center py-8 pl-10 pr-6">
            <div className="w-full flex flex-col items-start justify-center pt-4 px-0 pb-1 box-border shrink-0">
              ...
              <b className="self-stretch relative text-[13px] tracking-[-0.01em] font-inter text-aliceblue">Report your dorm manager</b>
            </div>
          </div>
          ...
          <div className="self-stretch flex flex-col items-start pt-6 px-8 pb-4 gap-8 text-[12px] text-dimgray font-inter">
            ...
            <div className="self-stretch flex flex-col items-start gap-2">
              <div className="self-stretch flex items-end py-0 pl-0 pr-4">
                <b className="self-stretch flex-1 relative flex items-center text-[12px]">Property & Maintenance Issues</b>
                <button 
                ...
                >
                  <div className="relative font-medium">
                    {selectedIssues.length === MAINTENANCE_ISSUES.length ? 'Deselect All' : 'Select All'}
                  </div>
                  <div className="h-4 w-4 relative">
                    <div className={`absolute h-full w-full rounded-sm ${
                      selectedIssues.length === MAINTENANCE_ISSUES.length ? 'bg-teal' : 'bg-whitesmoke-100 border border-gray-200'
                    }`} />
                    {selectedIssues.length === MAINTENANCE_ISSUES.length && <CheckIcon />}
                  </div>
                </button>
              </div>
                    ...
                    >
                      ...
                      <div className="flex-1 flex items-center">
                        <div className="flex flex-col items-start justify-center gap-0.5">
                          <b className="relative text-[12px]">{issue.label}</b>
                          <div className="relative text-[10px] font-medium text-dimgray">{issue.description}</div>
                        </div>
                      </div>
                      <div className="h-5 w-5 relative overflow-hidden shrink-0">
                        <div className={`absolute h-full w-full rounded-sm transition-all ${
                          selectedIssues.includes(issue.id) ? 'bg-teal' : 'bg-whitesmoke-100 border border-gray-200'
                        }`} />
                        {selectedIssues.includes(issue.id) && <CheckIcon />}
                      </div>
                    </label>
                  ))}
                </div>
