...
const FINANCIAL_ISSUES: ViolationOption[] = [
  { id: 'irregularities', label: 'Rent Collection Irregularities', description: 'Delayed deposits, missing payments.' },
  { id: 'unauthorized', label: 'Unauthorized Fees or Charges', description: 'Collects payments not related to tenant’s financial duties.' },
  { id: 'misuse', label: 'Misuse of Funds', description: 'Maintenance funds, deposits, etc.' },
];

const ReportManager2: FunctionComponent<StepProps> = ({ onNext, onCancel }) => {
  ...
  const handleSelectAll = () => {
    if (selectedIssues.length === FINANCIAL_ISSUES.length) {
      setSelectedIssues([]);
    } else {
      setSelectedIssues(FINANCIAL_ISSUES.map(v => v.id));
    }
  };
          ...
          <div className="self-stretch flex flex-col items-start pt-6 px-8 pb-4 gap-8 text-[12px] text-dimgray font-inter">
            ...
            <div className="self-stretch flex flex-col items-start gap-2">
              <div className="self-stretch flex items-end py-0 pl-0 pr-4">
                <b className="self-stretch flex-1 relative flex items-center text-[12px]">Financial Misconduct</b>
                <button 
                ...
                >
                  <div className="relative font-medium">
                    {selectedIssues.length === FINANCIAL_ISSUES.length ? 'Deselect All' : 'Select All'}
                  </div>
                  <div className="h-4 w-4 relative">
                    <div className={`absolute h-full w-full rounded-sm ${
                      selectedIssues.length === FINANCIAL_ISSUES.length ? 'bg-teal' : 'bg-whitesmoke-100 border border-gray-200'
                    }`} />
                    {selectedIssues.length === FINANCIAL_ISSUES.length && <CheckIcon />}
                  </div>
                </button>
              </div>
