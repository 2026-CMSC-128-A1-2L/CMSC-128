...
const SECURITY_ISSUES: ViolationOption[] = [
  { id: 'access', label: 'Allowing Unauthorized Access', description: 'Permitting non-residents or unverified individuals entry without protocols.' },
  { id: 'negligent', label: 'Negligent Security Practices', description: 'Leaving gates unlocked, ignoring broken locks, or disabling alarms.' },
  { id: 'reporting', label: 'Failure to Report Incidents', description: 'Not documenting or reporting thefts, break-ins, or harassment.' },
  { id: 'tampering', label: 'Tampering with Surveillance Systems', description: 'Disabling cameras or deleting footage without authorization.' },
];

const ReportManager4: FunctionComponent<StepProps> = ({ onNext, onCancel }) => {
  ...
  const handleSelectAll = () => {
    if (selectedIssues.length === SECURITY_ISSUES.length) {
      setSelectedIssues([]);
    } else {
      setSelectedIssues(SECURITY_ISSUES.map(v => v.id));
    }
  };
              ...
              <div className="self-stretch flex items-end py-0 pl-0 pr-4">
                ...
                >
                  <div className="relative font-medium">
                    {selectedIssues.length === SECURITY_ISSUES.length ? 'Deselect All' : 'Select All'}
                  </div>
                  <div className="h-4 w-4 relative">
                    <div className={`absolute h-full w-full rounded-sm ${
                      selectedIssues.length === SECURITY_ISSUES.length ? 'bg-teal' : 'bg-whitesmoke-100 border border-gray-200'
                    }`} />
                    {selectedIssues.length === SECURITY_ISSUES.length && <CheckIcon />}
                  </div>
                </button>
              </div>
